...
router.post('/find', (req, res) => {
    ...
    Brands.find(Vfilters).sort(Vsorts).count()
    .then(
        count => {
            rPerFind = count;
    ...
    //res.send(Vfilters);
    Brands.find(Vfilters).sort(Vsorts).skip(skypPage).limit(parseInt(req.body.limit))
...
router.put('/:id', (req, res) => {
    Brands.findOneAndUpdate(req.params.id, req.body)
    .then(x => res.status(204).send(x));
})

router.delete('/:id', (req, res) => {
    Brands.findOneAndDelete(req.params.id)
    .exec()
